...
abstract class AbstractSubSubDocumentController extends AbstractRestController
{
     ...
     *
     * @return SubSubDocumentServiceInterface
     */
    protected function getService()
    {
        return $this->get(
            'app.'.preg_replace('/controller$/', '', join('.', array_slice(explode('.', str_replace('\\', '.', strtolower(get_class($this)))), -3)))
        );
    }
    /**
    ...
    protected function handleFind(Request $request, $options = [])
    {
        return $this->returnResponse(
            $this->getService()->find(
                $this->getRequestService()->fetchRouteParameter($request, 'pParentId'),
